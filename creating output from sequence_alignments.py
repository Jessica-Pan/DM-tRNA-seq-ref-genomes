...
for i in range(3):
    ...
    for j in range(len(name_list)):
        temp.append("")
    to_write_to_mutation_outputs.append(temp)
...
for decoder in mutationIsodecoders:
    ...
    for elem in charging_list:
        ...
        charging_output.write(str(elem[0]) + "\t" + str(elem[1]) + "\t" + str(sum(elem)) + \
                              "\t" + mutationFraction + "\t")
    charging_output.write("\n")
    for ii in range(len(to_write_to_mutation_outputs)):
        for j in range(len(to_write_to_mutation_outputs[ii])):
            to_write_to_mutation_outputs[ii][j] += decoder
    for i in range(len(SEQ)):
        ...
        for mutList_index in range(len(mutLists)):
            ...
            if sum(mutList) > 0:
                ...
                to_write_to_mutation_outputs[0][mutList_index] += "\t" + mutFrac
            else:
                to_write_to_mutation_outputs[0][mutList_index] += "\t--"
            ...
            if i != len(SEQ) - 1:
                ...
                if nextTotal > 0:
                    ...
                    to_write_to_mutation_outputs[1][mutList_index] += "\t" + str(WC_mut)
                    output.write(str(stop) + "\t" + str(WC_mut) + "\t" + str(WC_mut + stop) + "\t\t")
                    to_write_to_mutation_outputs[2][mutList_index] += "\t" + str(WC_mut + stop)
                else:
                    to_write_to_mutation_outputs[1][mutList_index] += "\t--"
                    to_write_to_mutation_outputs[2][mutList_index] += "\t--"
                    output.write("\t\t\t\t")
            else:
                output.write("\t\t\t\t")
        output.write("\n")
    for ii in range(len(to_write_to_mutation_outputs)):
        for j in range(len(to_write_to_mutation_outputs[ii])):
            to_write_to_mutation_outputs[ii][j] += "\n"
    output.close()
for i in range(3):
    for j in range(len(name_list)):
        mutation_outputs[i].write(fancy_names[name_list[j]] + "\n" + to_write_to_mutation_outputs[i][j] + "\n\n")
